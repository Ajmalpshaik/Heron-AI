...
2. **On any machine with the .NET SDK: `python tools/check-fragments-compile.py`.** **Seventy-two**
   fragments now owe a real compile. This is `A9` and it is still the largest thing needing no Revit.
3. **With Revit open: start proving.** All 210 are `DRAFT`. [D-30](docs/DECISIONS.md) means a proof needs
...
## WHERE THIS STANDS, 2026-09-04 — 210 fragments, and seventy-two have never seen a compiler
...
| | |
|---|---|
| Fragments | **210**, every one `DRAFT`. 138 compiled on all eight Revit releases; **the newest 72 have not been compiled at all** — see `A9` |
...
> tools/check-fragments-compile.py`. It is now `A9` in the register. Seventy-two fragments were written
> on 2026-09-02, 2026-09-03 and 2026-09-04 in containers where the SDK could not be installed, because
...
> not the same thing and is not a substitute — though it has now caught **eight** real errors before
> they were compiled, across five distinct shapes. `A9` lists them.
