...
def _construct(offline):
    ...
    THE OFFLINE SWITCH IS PER-LOAD AND NEVER PROCESS-WIDE, and the first
    version of this got that wrong in a way worth writing down. It set
    huggingface_hub's HF_HUB_OFFLINE and TRANSFORMERS_OFFLINE around the call
    and put them back afterwards. That is a GLOBAL, and heron_brain.warm()
    starts the ENCODER's loader on another thread at the same moment - so on a
    machine with both stacks installed and neither cached, the encoder could
    observe this module's offline flag, fail its own download, and cache None
    for the life of the process. Retrieval would silently drop to `lexical` on
    a machine with a perfectly good network, which is the worst kind of
    degradation: correct-looking, permanent, and nothing to do with the module
    that caused it. Found by a review 2026-09-11.

    So there is no global state here at all. An older sentence-transformers
    that does not accept the keyword gets `absent` rather than a download - a
    person can still fetch deliberately with --fetch --yes, and nothing this
    module does can reach across into another loader.
    """
