...
| [`fragments/`](fragments/) | The library. **Thirty**, every one `DRAFT` — and **not one has met a real model**. All thirty do now **compile**, on all eight releases, via [`tools/check-fragments-compile.py`](../tools/check-fragments-compile.py) (2026-08-30). Its first run found one that never could have: `FRG-QA-001` had a value called `checked`, a reserved C# keyword |
