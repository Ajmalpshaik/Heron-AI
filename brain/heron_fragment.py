...
# HOW TO WRITE A VALUE OF EACH TYPE, in the words a person needs.
#
# IT LIVES HERE SO THERE IS ONE COPY. It was written for
# tools/generate-jobs.py, which emits job files a person fills in - and the
# same question is asked by anyone calling a capability over MCP, who until
# now had to read the add-in's own parser to answer it. One job took 18
# source lookups to learn what three inputs were called; every one of those
# answers was already in this function, in a developer CLI nothing on the
# MCP side could reach.
#
