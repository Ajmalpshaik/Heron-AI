...
"""
The 935 checks that compiled every day and ran on no day.
...
`tests/test_*.py`, so 935 checks over 414 fragments, as measured on 2026-09-26,
...
and prints **PASSED - 935 checks, 414 fragments, 106 carrying a guard**.

THE COUNT IT WAS LAST DESCRIBED WITH WAS 372. HANDOVER says the host "runs the
rewriter over all 372 fragments: 836 checks". The library is 414 now and the
host reports 934 checks run - so the host itself was right all along and the
