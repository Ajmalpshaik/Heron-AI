...
"""
The 926 checks that compiled every day and ran on no day.
...
`tests/test_*.py`, so 926 checks over 411 fragments, as measured on 2026-09-25,
...
and prints **PASSED - 926 checks, 411 fragments, 103 carrying a guard**.

THE COUNT IT WAS LAST DESCRIBED WITH WAS 372. HANDOVER says the host "runs the
rewriter over all 372 fragments: 836 checks". The library is 411 now and the
host reports 925 checks run - so the host itself was right all along and the
