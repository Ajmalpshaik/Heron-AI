...
"""
The 882 checks that compiled every day and ran on no day.
...
`tests/test_*.py`, so 882 checks over 395 fragments sat outside every count of
...
and prints **PASSED - 882 checks, 395 fragments, 91 carrying a guard**.

THE COUNT IT WAS LAST DESCRIBED WITH WAS 372. HANDOVER says the host "runs the
rewriter over all 372 fragments: 836 checks". The library is 395 now and the
host reports 881 checks run - so the host itself was right all along and the
