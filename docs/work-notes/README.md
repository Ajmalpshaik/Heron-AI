...
| ~~`handover/proving-session-2026-09-13.md`~~ | **RETIRED 2026-09-19 — read, and its durable half had already landed.** The handover for 2026-09-13: 199 → **204 `PROVEN`**, and six promotions reversed the same day after a review caught what the signing gate cannot see. What survives is [`FRAGMENT-ISSUES.md`](../FRAGMENT-ISSUES.md) rows 17–22 and [NEEDS-CHECKING](../NEEDS-CHECKING.md) **A16b** — including the finding that outranks the rest, that **a rollback which reports success sometimes does not happen** |
| ~~`handover/proving-session-2026-09-11.md`~~ | **RETIRED 2026-09-19 — it did not survive being read, which its own row said it would not.** The handover for 2026-09-10 into 2026-09-11: 175 → **197 `PROVEN`**, twenty-two promoted, **eleven defects** found. Its durable half is in [`FRAGMENT-ISSUES.md`](../FRAGMENT-ISSUES.md). Two things it carried were owed rather than done: **seven promoted proofs named a model they were not taken against**, and whether the risk gate belongs on `validate` |
...
| ~~`handover/mep-session-2026-09-16.md`~~ | **RETIRED 2026-09-19 — read, then deleted, as its own row instructed.** The handover for 2026-09-16: **298 → 310 `PROVEN`**, thirteen signed and promoted. **It was not a proving session, it was a JOB** — a copper refrigerant pipe type for split AC units in the owner's own naming convention and colour — and twelve fragments exist because that sentence could not be carried out with the library as it stood. Every one was proved against the model the job was done in rather than an arrangement invented for the proof, which is the stronger kind. What survives is [`FRAGMENT-ISSUES.md`](../FRAGMENT-ISSUES.md) rows 95–99 |
| ~~`handover/proving-session-2026-09-15.md`~~ | **RETIRED 2026-09-19 — scaffolding, read and removed.** The handover for 2026-09-14/15: **291 → 298 `PROVEN`**, seven signed and promoted. Its finding was that **not one of the seven was blocked by anything in a model** — every block was the machinery, and each a different shape. What survives is [`FRAGMENT-ISSUES.md`](../FRAGMENT-ISSUES.md) rows 89–94, [D-72](../DECISIONS.md) and [`tools/jobs/`](../../tools/jobs/) |
...
| [`BALANCE-OF-WORK.md`](BALANCE-OF-WORK.md) | **Active, and GENERATED — do not edit it by hand.** What is left to do, in one place: fragments never put in front of a model, skills, agents, register rows, open defects, unanswered questions and everything waiting on the owner. **It types no number of its own** — `python tools/balance-of-work.py --write` reads each figure back out of the tool that owns it, and every row names the command that derives it. Where it disagrees with a register, the register is right and this page is stale. **Retires when its rows reach zero — delete it and the tool together.** Added 2026-09-19 |
| [`investigations/2026-09-17-risk-crossings-not-deterministic.md`](investigations/2026-09-17-risk-crossings-not-deterministic.md) | **Active, and pinned open by a register.** The four runs that showed `check-risk-crossings.py` is not deterministic. **Do not delete while [`FRAGMENT-ISSUES.md`](../FRAGMENT-ISSUES.md) row **116** is OPEN** — that row names this file as the evidence its conclusion came from, because a conclusion is only re-checkable if the output it came from survives. **Added to this table 2026-09-19; it had been missing since the day it was written** |
| ~~`requested-fragments-2026-09-17.md`~~ | **RETIRED 2026-09-19 — it closed itself, and this is the note that proves a work note can.** The owner read out five lists of Revit jobs, roughly 290 of them; twenty-three came back with no route at all and were built. What survives: the twenty-three are in the library, that none of them has met a model is [NEEDS-CHECKING](../NEEDS-CHECKING.md) **A19**, and what is still worth building is in [`PROPOSALS.md`](../PROPOSALS.md) |
| [`plans/earlier-brain/README.md`](plans/earlier-brain/README.md) | **Active — planned 2026-09-23, being built package by package; the status is in [02 §0](plans/earlier-brain/02-work-packages.md).** What to take from the owner's earlier brain repository after everything in it was checked, and how: the folder's front page, its rules and its closure condition |
| [`plans/earlier-brain/01-findings.md`](plans/earlier-brain/01-findings.md) | **Complete as a record.** What was read, what Heron already does better, what was not taken and why, and the dangers the comparison found in Heron itself |
| [`plans/earlier-brain/02-work-packages.md`](plans/earlier-brain/02-work-packages.md) | **Active — the only copy of the packages' status.** Eleven packages, three waves, four owner decisions |
| [`plans/earlier-brain/03-owner-data.md`](plans/earlier-brain/03-owner-data.md) | **Active, and not product content.** The owner's site words, real questions, grayout and practice values, carried from his PC to go into his own stores |
| [`plans/earlier-brain/04-cloud-prompts.md`](plans/earlier-brain/04-cloud-prompts.md) | **Ready.** One cloud-session prompt per package, and the rules every session follows |
| [`plans/earlier-brain/05-pc-proving.md`](plans/earlier-brain/05-pc-proving.md) | **Waiting for the cloud packages.** What needs a real Revit, package by package |
