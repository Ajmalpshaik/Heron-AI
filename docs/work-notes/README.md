...
| [`investigations/jamwithai-repositories-2026-09-10.md`](investigations/jamwithai-repositories-2026-09-10.md) | **One row taken, the rest proposals.** Five repositories on one outside profile, read at the owner’s request for anything useful to the whole platform rather than only RAG. **Four ✅ take, three ⏸ owner’s call, the entire stack ❌ does not transfer** — every storage and orchestration piece in it needs a service or a server, which [D-01](../DECISIONS.md) forbids. The one worth having is a **fabrication check that calls no model at all**, which turns *no source, no claim* into a number measurable offline. It also **corrected [`plans/rag/00-structure.md`](plans/rag/00-structure.md)**: [34 §2.13](../34-patterns-adapted.md) had already measured a third retrieval stream at six settings and rejected it. Retires into [`docs/34`](../34-patterns-adapted.md) when the owner takes or refuses each row |
