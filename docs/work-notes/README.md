...
| [`plans/improvement-gate-execution-record.md`](plans/improvement-gate-execution-record.md) | **Active — the gate is built and piloted; four things are owed.** The ledger for the Improvement Gate: the Phase 0 audit of what Heron already had, one note per phase, and the evidence each produced. Its durable half is already moved — [D-68](../DECISIONS.md) and [D-69](../DECISIONS.md) carry the decisions, [34 §2.15–2.19](../34-patterns-adapted.md) the external patterns, [13 §3a](../13-testing-and-quality.md) the offline-vs-real-Revit line. What it still holds is **three defects recorded rather than fixed**, and the honest note that the gate has only ever been run by the session that wrote it, which [Golden Rule 7](../14-golden-rules.md) says is not enough. Retires with the three plan notes below once somebody who did not build it has used it |
