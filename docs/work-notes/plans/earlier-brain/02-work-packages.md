...
| **C3** | Add-in safety (Revit failure handling, "changed in central?") and an API signature lookup | cloud, then PC | 1 | — | **merged — [#306](https://github.com/Ajmalpshaik/Heron-AI/pull/306)**, 2026-09-23: the cloud part. Compiled on all eight releases, never run in Revit; the PC owes NEEDS-CHECKING **E19–E24** - **run on the owner's PC 2026-09-23**, Revit 2024 on a test project: **E19-E22 PASSED** (a warning dismissed and quoted, an error rolled back with Revit's words, nothing deleted or unlocked); **E23-E24 NOT RUN**, deferred by the owner - a test central is ready for them. Recorded in [group E](../../../needs-checking/group-e.md) |
| **C4** | Revit traps written into the fragments they affect | cloud, then PC | 1 | — | **merged** as [#309](https://github.com/Ajmalpshaik/Heron-AI/pull/309), 2026-09-23, on the owner's word — 25 fragment folders; code changed in 8, and 5 of those went `PROVEN` → `DRAFT` with their proofs kept. Owed on the PC: [NEEDS-CHECKING **Group AJ**](../../../needs-checking/group-aj.md), nine checks on a named model - **run in part on the owner's PC 2026-09-23**, Revit 2024 on a test project: **AJ1 FAIL**, the trap reproduced (5b-187); **AJ4, AJ5 NEEDS REVIEW**, the owner's calls; **AJ6** find-clashes PASS, its plumbing half NOT RUN; **AJ7 PASS**, the category wash found not landing (5b-188); **AJ2** NOT RUN, plumbing deferred by the owner; **AJ3, AJ8, AJ9** NOT RUN. Proofs: `connect-air-terminals` **accepted by the owner**; `color-by-parameter`, `override-graphics-in-view`, `find-clashes`, `center-room-tags` drafted, not signed; `dimension-grids-and-levels`, `find-dead-ends`, `find-system-islands` still owed |
| **C5** | The owner's words and standards — knowledge, not language | cloud, then PC | 2 | C2 merged | **dropped on the owner's word, 2026-09-24** - asked whether to build it after family creation, the owner said it is not needed. The 52 site words and the grayout standard stay recorded in [03](03-owner-data.md), loaded nowhere |
...
| **C7** | A fragment-writing agent | cloud | 2 | C3 merged | **merged** as [#330](https://github.com/Ajmalpshaik/Heron-AI/pull/330), on the owner's word 2026-09-24 (*"we need a helper to create a new tool on the spot"*): `.claude/agents/heron-fragment-author.md` and its `.codex` twin, gated by `brain/heron_matcher.py` and `brain/heron_generate.py`; `tests/test_agent_tools.py` now holds that an agent writing files names no tool that reaches Revit. Never run in a session yet. Owed on the PC: its first real use, one capability the owner names, written and then proved ([05](05-pc-proving.md)) |
| **C8** | Three fragments from the outside-library notes | cloud, then PC | 2 | C4 merged | not started |
| **C9** | Skills that carry a method — the design, then the HVAC methods | cloud, then PC | 3 | owner decision **D2** | **dropped on the owner's word, 2026-09-24** - asked whether to build it after family creation, the owner said it is not needed. The duct tools it would have needed (branch or riser, elbow, takeoff, end cap, reducer) fall with it |
| **C10** | Tagging and sprinkler methods | cloud, then PC | 3 | C9 merged | **dropped on the owner's word, 2026-09-24** - asked whether to build it after family creation, the owner said it is not needed - the owner named the sprinkler layout; the tagging method rode the same package and falls with it unless asked for on its own |
| **C11** | Family creation | PC | taken early | the owner's word, 2026-09-24 | **merged** as [#328](https://github.com/Ajmalpshaik/Heron-AI/pull/328), 2026-09-24, on the owner's word, **all still `DRAFT`** - taken ahead of C7 and C9 when the owner asked for family creation in Heron (*"ADD THIS IN HERRON MAKE IT PARTOF HERON"*): nine fragments and the `family-creation` skill, every one compiled on all eight releases, none yet run in Revit. Owed on the PC: [NEEDS-CHECKING **Group AN**](../../../needs-checking/group-an.md). Not built: voids, round shapes, materials, subcategories, nested families |
...
next step. When D2 gives skills a `steps:` list, this skill is the first to take it; AN11 records how well the
