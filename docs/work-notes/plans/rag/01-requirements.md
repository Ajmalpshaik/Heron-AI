...
| **R-08** | Chunking does not break the exact tokens BIM runs on — `OST_DuctCurves`, a parameter GUID, a clause number | [05 §4](../../../05-heron-brain.md) | **NONE** | the requirement most likely to be got wrong quietly. **R-68 is its sharper twin** and matters more |
...
| | Requirement | Source | State | Note |
|---|---|---|---|---|
| **R-66** | A chunk is embedded and indexed **with its heading path prepended** — *QCS 2014 → Section 21 → 21.3 Ductwork → 21.3.2* | [00 §3.8a](00-structure.md) | **NONE** | the published form of this cuts retrieval failures by **49%, and 67% with re-ranking** |
| **R-67** | That context is **read off the document's structure, never generated by a model** | [00 §3.8a](00-structure.md), D-24 | **NONE** | **the whole point.** A per-chunk model call would re-introduce exactly the cost D-24 removed. If this is ever replaced by a generated context, that is a **new decision**, not an optimisation |
| **R-68** | **A chunk is never split between a rule and its exception.** A candidate split immediately before *except*, *unless*, *provided that*, *save that* or *however* **is not a split point** | [00 §3.8b](00-structure.md) | **NONE** | **the worst thing this system could do.** Split *"ducts shall be insulated … except in conditioned spaces"* and Heron states the opposite of the requirement **with a citation attached**. QCS and Ashghal are written as rule-then-qualification throughout |
| **R-69** | A document's **chunk boundaries can be shown to a person before the document is trusted** | [00 §3.8c](00-structure.md), D-30 | **NONE** | no UI needed — printing one document's boundaries is the whole requirement. **A badly chunked standard does not fail; it answers, confidently, forever** |
| **R-70** | The **vector count is reported**, against the ceiling [D-23](../../../DECISIONS.md)'s choice implies — `sqlite-vec` brute-force stays fast below roughly **500,000** vectors | [00 §3.8d](00-structure.md) | **NONE** | Heron is at 360. The point is that the day it approaches the ceiling is a day somebody **notices**, rather than a day retrieval quietly gets slow |
