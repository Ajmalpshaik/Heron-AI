...
| `HERON-REVIT-FAM-012` | Revit Family Agent | Families, types, loading, placement. **Built 2026-09-17** — [`revit/Heron.Revit.Addin/RevitFamilies.cs`](../revit/Heron.Revit.Addin/RevitFamilies.cs), operation `list_families`. **Read-only, and the row's word “loading” is exactly what it will NOT do.** A family carries its own materials, and loading one overwrites the project's: six families once reset the pipe colour on a whole job **silently**, with no count moving and nothing warning — a real incident, and no check in this repository could have seen it, because the element count was identical either side. So `LOAD` is not an operation here and would be a separate entry at a much higher risk. **TYPES and INSTANCES are reported separately**, because *“how many of these are there”* has two answers in Revit and they differ by a lot. It names a **type placed nowhere** (carried in the file for ever, and in every type selector somebody then picks the wrong one from), an **in-place family** (reusable nowhere, remade next job) and a family with **many types and almost none placed** — the 10/2 pair is a rule of thumb and is stated in the answer so a reader can disagree with it. System families are **included and flagged**, not filtered, so the totals agree with the Project Browser. **Compiles 2020–2027, 0 warnings; never run in Revit** | T1 | MODIFY | — |
| `HERON-REVIT-VIE-013` | Revit View Agent | Views, sheets, view templates, visibility. **Built 2026-09-17** — [`revit/Heron.Revit.Addin/RevitViews.cs`](../revit/Heron.Revit.Addin/RevitViews.cs), operation `list_views`. **SHEETS ARE NOT IN IT, AND THAT WAS DECIDED RATHER THAN ASSUMED**: this row and [`HERON-REVIT-SHT-029`](#2-revit-engineering--36) both claimed them, and the owner settled it on 2026-09-17 — **the Sheet Agent owns sheets**. The overlapping words are left in both rows rather than edited out of one, because this row is quoted from the specification and deleting quoted text to record a decision loses the fact that a decision was ever needed. What this agent says about a sheet is only whether a view is *placed* on one, which is a fact about the view. **Read-only, although this row says MODIFY** — applying a template changes what every other person on the job sees in that view. Its argument is `list_phases`' other half: **a count taken in a view is a count THROUGH that view**, its template, discipline, detail level, filters and crop, so “412 ducts” is a fact about a view as much as about a model. It names a view with **no template** (nothing controls what it shows, the usual reason two plans of one floor disagree), a view on **no sheet**, and a **template nothing uses**. Placement is read from **both** things that place a view — a `Viewport` for a drawing and a `ScheduleSheetInstance` for a schedule — because counting only the first reports every schedule in the job as unplaced. **Compiles 2020–2027, 0 warnings; never run in Revit** | T1 | MODIFY | — |
| `HERON-REVIT-WRK-014` | Revit Workset Agent | Worksets, element ownership, checkout state. Ownership failure is a normal outcome. **Built 2026-09-17** — [`revit/Heron.Revit.Addin/RevitWorksets.cs`](../revit/Heron.Revit.Addin/RevitWorksets.cs), operation `list_worksets`. **Read-only**, same reasoning as `LVL-027`: nothing is created, opened, closed, borrowed or relinquished, because relinquishing somebody's borrowed element loses their unsynchronised work. The row's last sentence decided the voice of the whole file — **ownership is described, never judged**, because on a workshared job most of the model belongs to somebody else most of the time and a tool reporting that as a failure teaches its reader to ignore failures. **Ownership is SAMPLED and the sample is stated in every answer**: `GetWorksharingTooltipInfo` is one round trip to the central model per element, on Revit's own thread, so a full sweep would freeze the model for whoever is working in it. *Not workshared* is answered as its own sentence rather than as an empty list. **Compiles 2020–2027, 0 warnings; never run in Revit** | T1 | MODIFY | — |
...
| `HERON-REVIT-EXP-018` | Revit Export Agent | Exports — IFC, DWG, PDF, images, schedules. **Built 2026-09-17 as the SAFETY HALF ONLY** — [`revit/Heron.Revit.Addin/RevitExport.cs`](../revit/Heron.Revit.Addin/RevitExport.cs), operation `check_export`, registered at **Read**. **Nothing leaves the model**: no file is written, no path touched, nothing printed and nothing sent. That is not timidity — PUBLISH is the level for sending something OUT, and once a file has left there is no undo, the recipient has it, and somebody may already be building from it. A bad read wastes a minute; a bad export is a drawing on a site hut wall. So what exists answers **“would an export of this be worth sending?”**, which is the question people actually have and the one nothing in Revit answers until the file is already written. It looks for: nothing to export at all, **sheets that would print blank**, **links that are NOT LOADED** (an unloaded link exports as nothing, so the drawing goes out with that model missing and the geometry it was coordinated against absent), and rooms with no area feeding area schedules. It **reports and does not judge** — a coordination model with links deliberately unloaded is a legitimate thing to export. **The doing half is deliberately absent**: it needs a destination, an overwrite decision, a format and a person who meant it, and [27](27-build-order.md)'s rule is that a write path built before its safety path ships without one. **Compiles 2020–2027, 0 warnings; never run in Revit** | T1 | PUBLISH | — |
| `HERON-REVIT-IMP-019` | Revit Import Agent | Imports and links external files in. **Built 2026-09-17** — [`revit/Heron.Revit.Addin/RevitImports.cs`](../revit/Heron.Revit.Addin/RevitImports.cs), operation `list_imports`. **Read-only; nothing is imported.** **LINKED and IMPORTED are counted separately, and confusing them is the defect**: a linked CAD file stays outside the model and comes out cleanly, while an imported one is copied **INTO** it and never leaves — its layers, line patterns, text styles and fonts are in the project permanently, appear in every dialog from then on, and **deleting the import does not remove them**. It is the best-known way to ruin a Revit file and it is invisible: the two look identical in the drawing area, and only Manage Links and the Import category tell them apart. A view-specific import is flagged as such, because it shows on one drawing and nowhere else. **The count is of UNEXPLODED imports and says so** — an exploded import is loose lines and text, indistinguishable from work somebody drew, while its damage remains; nothing can count those afterwards, so no total is claimed. Revit links are counted but not described: [`LNK-015`](#2-revit-engineering--36) reads those properly and one fact belongs in one place. **Compiles 2020–2027, 0 warnings; never run in Revit** | T1 | MODIFY | — |
...
| `HERON-REVIT-SCH-026` | **Revit Schedule Agent** | Schedules — the way BIM people actually extract data. Read, create, modify, export ↗. **Built 2026-09-17** — [`revit/Heron.Revit.Addin/RevitSchedules.cs`](../revit/Heron.Revit.Addin/RevitSchedules.cs), operation `list_schedules`. **Read-only**: adding a field or changing a filter changes what everybody downstream is pricing from. **A schedule is a FILTERED VIEW**, and that is the whole point of the agent — its category, phase and filters each remove rows silently, so *“48 doors”* off a door schedule is the count of that category, on that phase, that passed that filter, and never the count in the model. So every schedule is reported with its filter count beside **what the model holds in that category**: the two numbers together are the answer and neither alone is. **This is the third place that argument has had to be made** — after `list_phases` and `list_views` — which is itself worth noticing. **It does not read the ROWS**: exporting a schedule's contents is `EXP-018`'s job at PUBLISH risk and is a different job from saying which schedules exist. Material takeoffs and key schedules are **included and flagged**, because a count that silently excluded them would be wrong in the direction nobody checks. The category tally is **one sweep of the model, not one per schedule** — the first draft swept per schedule, which on a real job is millions of iterations on Revit's own thread, the freeze the conventions forbid outright. **Compiles 2020–2027, 0 warnings; never run in Revit** | T1 | MODIFY | — |
| `HERON-REVIT-LVL-027` | **Revit Level & Grid Agent** | Levels and grids — the hosts almost everything depends on. Naming, elevation, extents ↗. **Built 2026-09-17** — [`revit/Heron.Revit.Addin/RevitLevels.cs`](../revit/Heron.Revit.Addin/RevitLevels.cs), operation `list_levels`. **Read-only, although this row says MODIFY** — that column is the highest level the row can require, and listing requires none of it; moving a level's elevation drags every element hosted on it, so a write would be a separate operation at its own risk. It reports the three states that are hard to see in Revit's own interface and expensive to miss: **two levels at one elevation** (legal, sometimes deliberate, and the usual reason something modelled on one cannot be found on the other — the Project Browser sorts by NAME, so the two sit nowhere near each other in it), **a level with nothing on it**, and **names that do not sort in elevation order**, which is reported as an observation rather than a fault. Every elevation is `AsValueString()` in the project's own units and never a raw double — Revit stores 3000 mm as 9.84 decimal feet, and an unlabelled 9.84 is read as millimetres by the next person. **Compiles 2020–2027, 0 warnings; never run in Revit** | T1 | MODIFY | — |
| `HERON-REVIT-RM-028` | **Revit Room & Space Agent** | Rooms, spaces, zones, areas. MEP loads and schedules depend on spaces existing and being bounded ↗. **Built 2026-09-17** — [`revit/Heron.Revit.Addin/RevitRooms.cs`](../revit/Heron.Revit.Addin/RevitRooms.cs), operation `list_rooms`. **Read-only**: placing a room, deleting an unplaced one or moving a boundary all change somebody's area schedule, which on most jobs is a contractual document. **Rooms and MEP spaces are reported side by side and the difference is stated** — rooms are the architect's, spaces the engineer's, and they go out of step whenever a partition moves and nobody presses the button. **UNPLACED and NOT ENCLOSED are counted separately**, because both give zero area, look identical in a schedule, and are completely different jobs: one sits nowhere in the model and still appears in every total; the other is placed and its boundary leaks, which one millimetre of gap will do. They are told apart by whether the element has a `Location` at all, asked of the element rather than inferred from the number. **REDUNDANT rooms are NAMED AND NOT COUNTED**, on purpose: Revit reports those through its Warnings list rather than on the room, and matching a warning by its description text would work in English and silently report zero everywhere else. The first draft carried a `roomsRedundant` counter whose detector returned `false` unconditionally — **a gate reporting zero for ever and being read as evidence the check works** — and it was removed rather than shipped; the answer now says the state is unchecked so a silence is not read as a clean bill. Areas are `AsValueString()` in the project's own units, never the raw square feet. **Compiles 2020–2027, 0 warnings; never run in Revit** | T1 | MODIFY | — |
| `HERON-REVIT-SHT-029` | **Revit Sheet Agent** | Sheets, numbering, titleblocks, revisions. Distinct from views ↗. **Built 2026-09-17** — [`revit/Heron.Revit.Addin/RevitSheets.cs`](../revit/Heron.Revit.Addin/RevitSheets.cs), operation `list_sheets`. **This row's “distinct from views” is now a decision rather than a proposal**: `VIE-013` claimed sheets too, and the owner ruled on 2026-09-17 that they are this agent's. **Read-only, although this row says MODIFY** — renumbering a sheet breaks every drawing reference pointing at it, across the whole set and every consultant's copy. It reports the four states that turn up **at issue**, which is the worst moment to find them: a sheet with **nothing on it** (the Project Browser shows an empty sheet and a full one identically, so it is invisible until it prints), a sheet with **no titleblock**, **more than one titleblock family** across the set, and **placeholder sheets**, which are counted separately rather than filtered out because a reserved number must never read as a sheet that is ready. **Sheet numbers are listed, never checked for uniqueness** — Revit refuses a duplicate at entry, so a gate here would report zero for ever and be read as evidence that it works. Revisions are reported and never interpreted: whether a sheet *should* carry one is a question about somebody's issue process. **Compiles 2020–2027, 0 warnings; never run in Revit** | T1 | MODIFY | — |
...
| `HERON-REVIT-DIM-031` | **Revit Dimension & Annotation Agent** | Dimensions, tags, text, keynotes. *"Create dimensions"* is one of the founding examples ↗. **Built 2026-09-17** — [`revit/Heron.Revit.Addin/RevitAnnotation.cs`](../revit/Heron.Revit.Addin/RevitAnnotation.cs), operation `list_annotation`. **Read-only**: creating a dimension, clearing an override or deleting a tag all change a drawing somebody may already have checked. **It exists to find the OVERRIDDEN DIMENSION** — one typed over with text instead of reporting what it measures. On the drawing it is indistinguishable from a real one. It says 2400 on a wall that is 2100, **survives every model change because nothing recomputes it**, is checked by nobody because it looks correct, and gets built. **Nothing in Revit lists them.** Multi-segment dimensions carry overrides on the segments, which are checked too — the same lie in a form that is harder to see. It also finds tags that lost their host: Revit warns once, at deletion, and never again. **Units are never re-derived** — values are Revit's own formatted strings taken as they are, because parsing a dimension back into a number is how a decimal-feet value would get loose through the one agent whose whole subject is numbers on a drawing. **No version branch for the 2025 `Dimension` split**: collecting by class matches `LinearDimension`, `RadialDimension` and `ArcLengthDimension` alike, which is [D-20](DECISIONS.md)'s preference for the spelling that is simply correct over a `#if` that hides one that is not. **Compiles 2020–2027, 0 warnings; never run in Revit** | T1 | MODIFY | — |
...
| `HERON-DEV-UNT-011` | Unit Test Agent | Runs unit tests. **Built 2026-09-17** - [`brain/heron_unit_test.py`](../brain/heron_unit_test.py), which sweeps every `tests/test_*.py` there is, discovered rather than listed. [D-75](DECISIONS.md) could not close this row on an existing file because the sweep was inline bash in `gates.yml`, which carries no header - so the file had to be written first. It reads exit **3 as WAITING rather than failing**, the distinction `check-gaps.py` draws, and carries each failure's last printed line so that three suites failing for one missing package reads as one finding rather than three. It runs; [`HERON-DEV-QA-016`](../brain/heron_qa.py) judges | T1 | — | — |
