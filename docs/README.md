...
> **All ten skills are `DRAFT`; 323 of the 396 fragments are `PROVEN`
> as of 2026-09-23** — this line said *every fragment is `DRAFT`* long after that stopped being true, so
...
**57 answered · 1 open, and nothing gates any phase.** The open one is [Q-58](OPEN-QUESTIONS.md), raised 2026-09-21 while building the installer: `HeronPaths` is the only thing allowed to build a Heron path, and the PowerShell script that installs Heron cannot call it. The last was [Q-57](OPEN-QUESTIONS.md), closed 2026-09-21 as [D-86](DECISIONS.md) (a write may declare a question only when answering it REQUIRES the write) — may a fragment or skill that WRITES declare a question?
...
| `Q-49` | ✅ Heron had **no hooks of any kind** — every `check-*.py` gate ran only when a person typed it, and this repository had already paid for that once ([32 §4](32-master-architecture-reconciliation.md)). Closed by the owner 2026-09-09: *"yes add the hooks"*. Since 2026-09-23 they are wired from [`.claude/settings.json`](../.claude/settings.json), so every session runs them — [`heron-guard`](../.claude/skills/heron-guard/SKILL.md) and [`heron-session`](../.claude/skills/heron-session/SKILL.md) |
