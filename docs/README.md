...
**42 answered · 11 open, and nothing gates any phase.** `Q-43` to `Q-48` were all opened on
...
| 33 | [The External Repository Research Matrix](33-external-repository-research.md) | **The fifteen projects [`HERON_AI_MASTER_ARCHITECTURE.md`](../HERON_AI_MASTER_ARCHITECTURE.md) §10 asks about, read on 2026-09-09.** The headline is not an adoption: **four of them independently arrived at designs Heron already has** — RRF-fused hybrid retrieval, deterministic-before-model, compress the answer never the question, and incremental-by-content-hash. Nine needed no action. **Nothing is adopted as code**, and one project is **AGPLv3** against Heron's Apache 2.0 |
