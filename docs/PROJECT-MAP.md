...
| `.claude/` | House rules as skills, and the prototype agents that keep them. **The only tree of HOUSE RULES** — `brain/skills/` above is Heron's own skills, the jobs it does for a modeller, and the two share a word and nothing else. **`agents/heron-*.md` are not house rules**: they are Heron's own background agents, the *subagents* [D-01](DECISIONS.md) says Heron supplies, and they live here because this is the folder the host reads them from. The first is `heron-model-auditor`. A background agent carries a read-only tool list, held by `tests/test_agent_tools.py`. **`settings.json` wires the development hooks every session runs** - the boundary guard, the session-start line and the *has main moved?* note, each writing its decisions to a diary outside the repository. They are for developing Heron; a modeller never meets them | [`.claude/skills/README.md`](../.claude/skills/README.md) · [`heron-session`](../.claude/skills/heron-session/SKILL.md) |
