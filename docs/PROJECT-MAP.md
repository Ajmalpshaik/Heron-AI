...
| `.claude/` | House rules as skills, and the prototype agents that keep them. **The only tree of HOUSE RULES** — `brain/skills/` above is Heron's own skills, the jobs it does for a modeller, and the two share a word and nothing else. **`agents/heron-*.md` are not house rules**: they are Heron's own background agents, the *subagents* [D-01](DECISIONS.md) says Heron supplies, and they live here because this is the folder the host reads them from. The first is `heron-model-auditor`; `heron-fragment-author` writes a new fragment at DRAFT when nothing in the library does the job. A background agent carries a read-only tool list, and an agent that writes files carries no tool that reaches Revit - both held by `tests/test_agent_tools.py`. **`settings.json` wires the development hooks every session runs** - the boundary guard, the session-start line and the *has main moved?* note, each writing its decisions to a diary outside the repository. They are for developing Heron; a modeller never meets them | [`.claude/skills/README.md`](../.claude/skills/README.md) · [`heron-session`](../.claude/skills/heron-session/SKILL.md) |
| `.codex/` | The same agents for a second host, pointing at the same skills. **A `.codex` copy carries no tool list**, so there the read-only rule is an instruction in the agent's text, not a restriction | `.codex/config.toml` |
...
| Know **what to run before pushing** | [`heron-ship`](../.claude/skills/heron-ship/SKILL.md) skill | It names the four gates you run before pushing - and every gate CI's job of that name actually decides on, which [`.github/workflows/gates.yml`](../.github/workflows/gates.yml) lists - how to state a change's intent and capture its evidence, and the checks that need a machine this container has not got |
