...
| Agents | **213 of 250 have code** as of 2026-09-16, 4 host-provided by D-01, **33 left** — `python tools/agent-count.py`. **Sixteen departments are complete.** Phase 0/1's agent list is COMPLETE. The 33 split cleanly and the split is the useful part: **16 wait on one sentence from the owner** — **F23** four Documentation rows, **F27** five Development rows, **F31** five Standards rows, **F15/F17** two Naming rows — and **12 Revit Engineering rows are open work**. **Two were built on 2026-09-16**: `HERON-REVIT-API-020`, which reasons ABOUT the API rather than touching a model, and `HERON-REVIT-SYS-030`, the MEP System agent. **The twelve are no longer blocked on a compiler** — [§30](30-compiling-away-from-windows.md) is a five-minute install and every one of them builds on 2020–2027 once it is done. What they are blocked on is a MODEL: each is `MODIFY` in the register, which that column defines as the HIGHEST permission it can require, so each is built READ-FIRST exactly as `RevitLinks`, `RevitPhases` and now `RevitSystems` were — and none of the three has ever run |
| MCP tools | **22** as of 2026-09-16. The three a modeller will feel are **`revit_links`** (2026-09-14), **`revit_phases`** (2026-09-15) and **`revit_systems`** (2026-09-16), and they answer the same class of question: a number about a model is only a number about the part of it you asked for. Elements inside a LINK are not in the host document; a count is a fact about a model AND a phase AND a design option; and an MEP element **connected to nothing** is on no system, carries no flow, and is missing from every total downstream without anything saying so. All three read-only. **Define a new tool ABOVE `if __name__ == "__main__"`** — `server.run()` never returns, so a decorator below it never runs and the tool is simply absent while the registry still declares it. That was a round-one Codex finding and `tests/test_api_docs.py` now checks it. Derive the count: `grep -c '^@server.tool()' mcp/server/heron_mcp_server.py` |
