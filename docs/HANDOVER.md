...
| Agents | **212 of 250 have code** as of 2026-09-16, 4 host-provided by D-01, **34 left** — `python tools/agent-count.py`. **Sixteen departments are complete.** Phase 0/1's agent list is COMPLETE. The 34 remaining split cleanly and the split is the useful part: **16 are waiting on one sentence from the owner** — **F23** four Documentation rows, **F27** five Development rows, **F31** five Standards rows, **F15/F17** two Naming rows — and **13 Revit Engineering rows need a machine**, not a decision. **`HERON-REVIT-API-020` was built 2026-09-16** and was the one Revit Engineering row buildable here: it is `READ`, and it reasons ABOUT the API rather than touching a model. **The other thirteen are all `MODIFY` and all C# in `revit/Heron.Revit.Addin/`** — and a container that has not installed the SDK cannot compile them, so writing them there would be writing code nobody could check. **That is a five-minute install, not a wall** ([§30](30-compiling-away-from-windows.md)); it was done on 2026-09-16 and every project built on all eight releases. What still stops the thirteen is not the compiler — it is that each is a write against a model nobody here can open. **API-020 is what makes writing them safer**: it refuses a member a target release removed, which is the `ElementId.IntegerValue` case caught before the C# rather than after the eighth release fails to build |
