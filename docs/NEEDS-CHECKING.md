...
| **J9** | Write a fragment whose lambda calls itself with no floor - `Func<int,int> f = null; f = n => { return f(n + 1); };` - and run it through `run_fragment_read` | **A FINDING, and Revit still running.** Expect `fragment_threw` naming *insufficient stack*. **The FAIL is the whole point of the row: if Revit vanishes, the guard did not take.** Added 2026-09-16 with [FRAGMENT-ISSUES row 104](FRAGMENT-ISSUES.md). `HeronStackGuard` is proved over the whole library by `tests/Heron.StackGuard.TestHost` - 836 checks, 91 fragments guarded - but **that proves the rewriting, not the catching.** No test in this repository can tell you what the CLR does when the stack actually runs out; only a running Revit can. **Needs the add-in rebuilt and deployed first** - the guard is add-in C#, not fragment C#, so it costs a close-build-deploy-reopen. Run it on a BLANK project: the expected outcome is a clean refusal, and the whole reason the row exists is that the unexpected one takes the model with it |
