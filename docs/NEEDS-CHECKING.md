...
and read rather than the total trusted. **Two rows have been added since that recount: `A10` and `A11`, both on
2026-09-11**, so these three totals are each two short and the two sentences below them are about the
...
model work). **Group A was finished on 2026-09-06**, all nine rows — and reopened on 2026-09-11 by `A10`, which needs `huggingface.co` reachable rather than a Windows machine, and by `A11`, which needs a compiler and a model.
...
**Does NOT need Revit** — except `A11`, which does. This group is **nine rows done of ELEVEN** — `A10` and `A11` were both added 2026-09-11 and are the two open. `A6` and `A9` both closed on
...
at one. **`A11` was added the same day and is the first row in months that does need a compiler.** **`A10` was added on 2026-09-11 and needs a network rather than a machine** — the re-ranker
...
| **A11** | `dotnet build`, then in Revit run any Heron read tool and look at the reply the server receives — **on the owner's PC** | **The project key reaching the server, which is what names a project knowledge store.** `RevitOperations.ProjectKey()` was added on 2026-09-11 and **has never been compiled**: this container has no .NET SDK. The add-in already computed this identity for its own preview/commit pairing (`RevitWrite.DocumentKey`) and never sent it, so `heron_standards` either skipped the project scope or would have named a store after a file path. Pass looks like: the build clean on 2020 and 2027, a `projectKey` field present in the `count_elements` and `select_by_category` replies, its value **the same string across a save, a rename and a move of the model**, and `pinned.project_key` non-`None` in a conversation that has run **only read tools**. `ProjectInformation.UniqueId` is the same expression `RevitWrite.cs` already uses on every release 2020–2027 — that is evidence the member exists, and it is **not** a build |
