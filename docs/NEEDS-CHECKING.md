...
| **E12** | Then click into a **second open project** and ask for the same change | It **refuses** and names both models — the guard must survive the E11 fix, because the user really does change model. **Then look at the second model.** On this path the refusal is raised at `heron_mcp_server.py` AFTER `run_fragment_write` has already called `group.Assimilate()`, so the change may be in the wrong project already and the sentence *"Nothing has been sent to Revit"* would be false. `revit_select_by_category` does not have this problem: it sends `expectProject` and [`RevitOperations.cs`](../revit/Heron.Revit.Addin/RevitOperations.cs) refuses **before** it touches the selection. The fragment write path sends nothing and has no such gate — found 2026-09-15 while fixing E11, not yet closed |
