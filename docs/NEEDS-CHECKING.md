...
> | **Adding to it** | A row names the **command**, the **machine it needs**, and what **pass looks like** — including what a FAIL would prove, which is worth the same. **Its ID is bold, and used once in this file** — look for the letters in every form, bold, backticks and plain, before a new group takes them. **A results table names the row's ID in plain text, and a row that passed is struck at both ends** — the three readers of this file count by the bold ID, so anything else is a check the owner's queue cannot see, sees twice, or shows him again after it passed. FRAGMENT-ISSUES row 5b-156 |
...
|---|---|---|
| ~~**R1**~~ | Read D-23 to D-27 back to Ajmal — [full row](needs-checking-archive/group-r.md#row-r1) | **DONE 2026-08-29 — and it covered D-23 to D-43, not just the five.** |
...
| ~~**R2**~~ | Pay particular attention to D-26 and D-32 — [full row](needs-checking-archive/group-r.md#row-r2) | **DONE 2026-08-29.** |
...
| ID | Do this | Pass looks like |
|---|---|---|
| ~~**AA1**~~ | Deploy **both** proofs with the AI Bridge add-in also installed. Start Revit. **Screenshot.** | **PASSED 2026-09-21 on the owner's PC** - the verdict is in *Group AA was RUN*, below. *It was to look like:* **TWO tabs**: `Heron` and `Heron Doc`. The `Heron` tab carries **both** an `AI Bridge` panel and a `Tools` panel - **ONE tab, two panels**. **The failure to watch for is two tabs both reading `Heron`**, which on screen looks almost right: that would mean `CreateRibbonTab` made a second one instead of finding the first, and R-35 is broken. The `Heron` tab's **three existing buttons must still work** - press each one |
| ~~**AA2**~~ | **TOOLS ONLY.** `.\tools\deploy-addin.ps1 -RevitVersion 2024 -Product heron-bridge -Remove`, leaving only the Tools proof. Start Revit. **Screenshot.** | **PASSED 2026-09-21 on the owner's PC** - the verdict is in *Group AA was RUN*, below. *It was to look like:* A `Heron` tab **exists**, carrying the `Tools` panel and **NO `AI Bridge` panel**. **THIS IS THE CASE EXPECTED TO BREAK FIRST** and the one [R-34](work-notes/plans/plugin-extension/01-requirements.md) promises - a site modeller who wants Heron's tools and refuses the AI. If **no tab appears at all**, the tools piece assumed it loaded second and something has to create the tab. A FAIL here is worth as much as a pass: it is the answer Stage 2 was built to get |
| ~~**AA3**~~ | **AI BRIDGE ONLY.** `-Product heron-tools -Remove`, then `-Product heron-bridge` to reinstall the add-in. Start Revit. **Screenshot.** | **PASSED 2026-09-21 on the owner's PC** - the verdict is in *Group AA was RUN*, below. *It was to look like:* The `Heron` tab exactly as it is today - one `AI Bridge` panel, three buttons, no `Tools` panel. This is the control case: it proves the proofs left nothing behind |
| ~~**AA4**~~ | `.\tools\deploy-addin.ps1 -RevitVersion 2024 -Product heron-doc -Remove`, then start Revit | **PASSED 2026-09-21 on the owner's PC** - the verdict is in *Group AA was RUN*, below. *It was to look like:* **`Heron Doc` is gone and `Heron` is untouched.** This is the uninstall story proved before any uninstaller exists. Check the Addins folder afterwards: no `Heron.Doc` folder, no `Heron.Doc.addin`, and **no `.old` anything** - [R-38c](work-notes/plans/plugin-extension/01-requirements.md) |
| ~~**AA5**~~ | Do AA1 on **2020** as well as on a modern release | **PASSED 2026-09-21 on the owner's PC** - the verdict is in *Group AA was RUN*, below. *It was to look like:* 2020 is `net472` and 2027 is `net10.0-windows`. The ribbon API is the same across both, but this has never been run, and `Z9` is the precedent for checking rather than assuming |
...
| ID | Do this | Pass looks like |
|---|---|---|
| ~~**AB1**~~ | Run `HeronInstaller.exe`. Look at it | **FAILED, FIXED, AND THE RE-RUN PASSED - 2026-09-21 on the owner's PC.** The verdict is in *Group AB was RUN*, below. *It was to look like:* **A window appears** and can be read. The Revit releases on the PC are listed as tick boxes, all ticked. `Heron` carries **two ticks under it** - the AI Bridge and the tools - and every other tab is one tick, whole tab. **Screenshot.** The failure to watch for is text cut off or wrapped into nonsense at a display scaling nobody tested |
| ~~**AB2**~~ | Look at the rows that **cannot** be ticked | **PASSED 2026-09-21 on the owner's PC** - the verdict is in *Group AB was RUN*, below. *It was to look like:* Each is greyed **and the sentence saying why is printed under it**, not hidden in a tooltip - [R-10](work-notes/plans/plugin-extension/01-requirements.md). Today that is the two Stage 2 throwaways (*"Not ready yet..."*) and Heron MEP (*"Not built yet..."*). **Neither sentence may use the words SHIPPED, PROVING or PLANNED** - those are this repository's vocabulary, not a modeller's |
...
| ~~**AB7**~~ | Press Install with **nothing ticked**, and again with **no Revit version ticked** | **PASSED 2026-09-21 on the owner's PC** - the verdict is in *Group AB was RUN*, below. *It was to look like:* It says which of the two it is and changes nothing. Neither is an error and neither is silence |
...
## Group AD - Revit traps seen elsewhere, written into the fragments they affect (earlier-brain plan, package C4)
