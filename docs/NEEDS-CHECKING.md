...
| **E14** | `revit_select_by_category`, then `revit_change`, **same model, no switching** | The change **runs**. Both tools must produce the **same** key for one model — while the fragment reply carried only the title, these two disagreed and the second refused a model it was already working in, naming it on both sides of *"but"* |
