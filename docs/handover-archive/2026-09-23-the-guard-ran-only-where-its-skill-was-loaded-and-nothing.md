...
- **New:** `.claude/settings.json` now runs three hooks in every session. `heron-guard` moved there from
  its skill's frontmatter, where it had run only in sessions that loaded the skill (row 5b-164). The new
...
- **Mistake worth not repeating:** a test that pipes JSON into a hook through `json.dumps` escapes every
  ...
  (row 5b-165) until the payload was sent as raw UTF-8.
- **To do:** NEEDS-CHECKING **Group AI** - four checks on the Windows PC, no Revit. Rows 5b-167 (the cloud
  setup script installed nothing, so the `heron` MCP server could not start), 5b-168 and 5b-169 are open
