...
owner's word in [PR #PRNUM](https://github.com/Ajmalpshaik/Heron-AI/pull/PRNUM). All of it is for
...
- **Fixed:** [`tools/cloud-setup.sh`](../../tools/cloud-setup.sh) installs past a refused `apt-get update`
  ...
  **It reaches no session until the owner pastes it into the environment's Setup script box -
  NEEDS-CHECKING Group AJ.** The `heron` MCP server still will not connect in a cloud session: it exits
...
- **Found, recorded, not fixed:** five Python files resolve a Windows special folder outside the two path
  owners and the structure gate cannot see them (row 5b-173); `PROJECT-MAP.md` says CI decides on ten
  gates, and it is eleven (row 5b-174).
