...
| **131** | **`check-routing.py` REWRITES THE IDENTITY TABLE IT READS, AND REMOVES DECLARED UTTERANCES WHILE DOING IT.** Measured 2026-09-19, reproducibly, three times. Two sentences were added to `count-elements`'s `utterances` to fix a live crossing (see the cell to the right). After the official rebuild - `python brain/heron_fragment.py` then `heron_search.index(store)` - both resolved by IDENTITY: **`how many sprinklers on level 2 -> FRG-ELE-002`** and **`give me the number of fire dampers -> FRG-ELE-002`**, confirmed in the `identities` table AND live through `heron_lookup`, which answered *"matched by identity, provided by FRG-ELE-002"* for both. **RUNNING `python tools/check-routing.py` THEN MAKES BOTH PHRASES VANISH FROM THAT TABLE** - not `__ambiguous__`, which is the deliberate miss the indexer writes on a clash, but **ABSENT**. Rebuild and they come back; run it again and they go. **NO CLASH EXPLAINS IT.** `FRG-ELE-004` (`place-family-instances`), the fragment that was winning the sentence, does NOT declare it - its eight utterances are `place the air terminals`, `put the sprinklers in`, `add the diffusers` and five more, none of them a question. So the phrase has exactly one claimant and should be a clean identity row. **THE TOOL IS NOT READ-ONLY AND ITS OWN PROSE SUGGESTS IT IS** - it calls `SEARCH.index(store)` and `EMBED.index(store)`, both writers, while its header describes reading the store. **THIS CLASS OF FAULT IS ALREADY KNOWN ONE DOOR ALONG:** `check-risk-crossings.py` carries a comment recording that *one sweep against a private copy changed the store this run had already moved*, and prints the store's hash before and after for that reason. `check-routing.py` prints no such thing. **THE CONSEQUENCE IS THAT A ROUTING FIX DOES NOT SURVIVE A ROUTING CHECK** - the yaml stays right and the store silently goes back to the dangerous answer until somebody rebuilds. | **OPEN.** Found 2026-09-19 while fixing a crossing and verifying it. The fragment files are the source of truth and are correct; what is volatile is the store. Two things worth doing, neither done here: have `check-routing.py` report the store hash before and after the way its sibling does, and settle whether a checker should write to the shared store at all. Note the tool DID register half the fix - its crossing count went 2 to 1, so `give me the number of fire dampers` is genuinely repaired by its own measurement |
