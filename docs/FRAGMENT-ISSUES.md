...
| **120** | **[ROW 108](#)'s SWEEP, RUN: FIFTEEN FRAGMENTS DECLARE A REFUSAL AS A LIST THAT CAN ONLY EVER HOLD ONE REASON, AND ALL FIFTEEN LOSE TEXT TO THE 60-CHARACTER CUT.** Row 108 asked for *a sweep of the fragments whose only refusal is a single reason in a one-item list* and left it unrun. Method: every provide declared `IList<string>` whose name is a refusal, where **no `.Add(` sits inside a `for`/`foreach`/`while`** - so the list is bounded by hand-written branches rather than by a collection, and an if/else chain fires at most one of them. **The fifteen**, with the longest refusal each would have to fit in 60 characters: `set-view-section-box` **451**, `set-schedule-filters` **335**, `create-electrical-circuit` **293**, `set-view-template-control` **278**, `assign-location-data` **268**, `create-mep-system-type` **238**, `add-revision-cloud` **227**, `create-view-filters-by-value` **213**, `create-flex-duct` **199**, `rename-workset` **196**, `create-grids` **190**, `create-room-elevations` **183**, `create-view-template-from-view` **173**, `place-family-on-face` **105**, `array-elements-radial` **70**. **TWO WERE READ BY HAND TO CHECK THE PATTERN RATHER THAN TRUSTED:** `rename-workset` is a strict if/else chain of eight, `set-view-section-box` a chain of four with a mutually exclusive pair nested in its last branch. Both produce exactly one reason, ever. **THE FIRST VERSION OF THIS SWEEP RETURNED 189 AND WAS WRONG** - its brace tracking counted a `.Add(` inside a `foreach` as outside one, and `unload-links` and `set-category-visibility` were the two that showed it, both plainly inside loops over links and categories. A sweep that flags half the library is reporting its own bug | **OPEN as the owner's call, and it is NOT a code fix somebody can just make.** The change is `IList<string>` to `string` in each `fragment.yaml`, and **13 of the 15 are PROVEN** - so it would stale 13 proofs to improve a message. Worth doing for `set-view-section-box` and `set-schedule-filters` alone, where 451 and 335 characters come back as 60; hard to justify for `array-elements-radial` at 70. **Nothing was changed here**: `fragment.yaml` is not this session's to edit, and re-proving 13 fragments is a decision about a day's work rather than a defect to repair |
| **121** | **[ROW 112](#)'s FIX WAS APPLIED IN ONE OF THE THREE PLACES THAT CARRIED THE FALSE PROMISE, AND THE OTHER TWO STILL READ AS THEY DID.** `select-types` promised *null or empty means every category* in three separate texts: the `needs:` comment in `fragment.yaml`, the `purpose:` prose above it, and the header of `impl/any/fragment.cs`. Row 112 was recorded on 2026-09-16 and the `needs:` comment was corrected then - carefully, naming the binder and the row. **The other two were not touched and neither is marked**, so a reader who opens the implementation, or who reads the purpose text the lookup returns, is told the same false thing that caused the row. Found by grep on 2026-09-19: `fragment.yaml:39`, `impl/any/fragment.cs:12`. **THE BEHAVIOUR IS THE HALF THAT IS RIGHT** - the loop below the comment genuinely treats an empty set as every category (`if (wantedCategoryIds.Count > 0)`), and the binder genuinely refuses to let a caller reach it. Nothing is broken; three texts describe one contract and one of them has been corrected | **PARTLY FIXED 2026-09-19.** `impl/any/fragment.cs:12` rewritten to say at least one is required, why the binder is right to refuse, and that the empty branch below it is deliberate but unreachable. **`fragment.yaml:39` IS STILL FALSE and was deliberately not touched** - `fragment.yaml` is not this session's to edit. One line, no model needed: *Passing none means every category, which is the honest answer to "list the types in this model" and is deliberately allowed.* **The general lesson is the row**: a contract written in three places is corrected in one, and grep is the only thing that finds the other two |
