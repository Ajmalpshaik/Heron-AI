...
| **104** | **REVIT CANNOT RENAME A PHASE AT ALL, THROUGH EITHER DOOR - AND THE COMPILE, THE SURFACE CHECK AND THE DOCUMENTATION ALL SAID IT COULD.** Measured 2026-09-16 on `PIPE` (3,332 elements), Revit 2020, session 35904, proving the new `RENAME_PHASE`. `Element.Name` is declared `get/set` on `Element` and `Phase` inherits it; `check-api-surface` passes it on all eight releases and is RIGHT to - the member exists. Revit throws on the setter: **"This element does not support assignment of a user-specified name."** The phase's name also lives in `BuiltInParameter.PHASE_NAME`, which is the second door every workaround reaches for: **`IsReadOnly` is `true` on it.** Both refusals came back in the same run, verbatim, because the fragment tries the documented route first and reports what each one said. **NOTHING SHORT OF RUNNING IT COULD HAVE FOUND THIS.** `check-fragments-compile` passes on 2020-2027, `check-api-surface` passes on 2020-2027, and both are correct - a member that exists and throws is exactly what `tools/api-surface/changes.json` lists under `cannotSee`. **IT IS THE SAME SHAPE AS A FACT `REPORT_PHASES` ALREADY CARRIED** and nobody read across: Revit exposes no CREATE call for a phase on any release either. A phase is not an editable object from the API; it is read. **THE NEGATIVE LEG WAS CLEAN** - `no phase called 'ZZZNoSuchPhaseAnywhere'. This model has: Existing, New Construction` - so D-30's second leg holds and its FIRST can never be non-empty on this release. The fragment stays `DRAFT` and must not be promoted. **IT IS STILL WORTH KEEPING, AND THAT IS NOT A CONSOLATION.** Before it, *"rename this phase"* resolved to `SELECT_BY_PHASE` or to `SET_ELEMENT_PHASE`, **which MOVES ELEMENTS BETWEEN PHASES** - a write to the wrong property that looks like it worked. It now resolves to a fragment that says Revit will not do this and to rename it under Manage, Phases. **ONLY 2020 WAS MEASURED.** Whether 2021-2027 refuse too is not established, and a declared absence is not the same as none. | **OPEN.** Found 2026-09-16. Not a defect in the fragment - a limit in Revit, recorded so the next person does not spend the afternoon finding it again. Worth one run on 2024 or 2027 to see whether the limit is the release or the object |
| **105** | **A REFUSAL RETURNED AS A `IList<string>` IS CUT TO 60 CHARACTERS, AND IT HID THE ONE FACT THE RUN EXISTED TO FIND.** Same run as [row 104](#). `RENAME_PHASE` declared `refused` as `IList<string>` - the shape `RENAME_WORKSET` and most of the library use - and the proof record came back `refused: 1 item(s) ['Existing' could not be renamed to 'HERON PROOF PHASE' - Thi...]`. **Three characters of the answer.** `RevitFragment.Describe` returns a `string` WHOLE and passes every item INSIDE a collection through `Shorten`, which is `text.Length > 60 ? Substring(0, 60) + "..."`. Both behaviours are right on their own: a list preview exists so a proof can see WHAT came back without printing forty rows. **THE MISTAKE IS DECLARING A ONE-OF-ONE AS A LIST.** The fragment's checks are a single if/else chain, so at most ONE reason is ever produced - the list described the fragment wrongly before it truncated anything. Changed to `string`, and Revit's two refusals then arrived in full and in one line. **THE RULE WORTH CARRYING: where the MESSAGE is the answer rather than a sample of many, declare it a string.** A count is honest about a list; its contents are not. Worth a sweep of the other fragments whose only refusal is a single reason in a one-item list. | Fixed in `RENAME_PHASE` the same day. The executor was not touched and should not be - the 60-character sample is right for a real list |
