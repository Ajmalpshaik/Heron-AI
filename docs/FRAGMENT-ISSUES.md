...
| **126** | **`revit_select_by_category` NOW ANSWERS `18 ducts` TO EVERY CATEGORY THERE IS, AND THE REFUSAL [ROW 110](#) CALLED CORRECT IS GONE.** Measured 2026-09-19 on `Project1 work_ajmal.al`, Revit 2024, session 20472, four requests in a row: *"select the ducts in the view 1 - Mech"* -> **Selected 18 ducts**; *"select the title blocks"* -> **Selected 18 ducts**; *"select the grids"* -> **Selected 18 ducts**; *"select the pipes"* -> **Selected 18 ducts**. **ROW 110 RECORDED THE PIPES CASE ANSWERING** *"Heron does not know the category 'pipes' yet. It understands: duct, duct curves, ducts, ductwork"* **- and that sentence no longer appears.** So the hard-coded four-row table has stopped refusing and started answering, with the wrong category and no sign anything was wrong. **THIS IS STRICTLY WORSE THAN THE DEFECT ROW 110 FILED.** That row's whole argument was that *the refusal is correct and the inconsistency is the defect* - a modeller told **I do not know pipes** by a system that had just counted 120 of them. Being told **Selected 18 ducts** instead is the same ignorance wearing a success message, and it is the shape this file keeps naming: a tool that cannot do something reporting that it did. **THE TITLE BLOCK CASE IS THE ONE THAT PROVES IT IS NOT AN EMPTY-CATEGORY FALLBACK**: the model HAS exactly one title block, and the fragment `select-by-category-name` found it in the same minute - `elements 1 [A1 metric]`, `resolvedTo Title Blocks`, `nearMisses 0`. So the content was there and the tool did not look for it | **OPEN. Found 2026-09-19 while arranging [row 123](#)'s proof, and it changed how that proof had to be taken** - the title block had to be reached through the FRAGMENT because the TOOL could not name it. **Add-in C#, so it costs a rebuild and a Revit restart** - batch it with [row 110](#), which is the same four-row table and the same deeper question: why two category resolvers exist. **NOT ESTABLISHED, and it is the first thing to read**: whether the refusal was removed, or whether the request never reaches the table because something upstream now resolves every phrasing to `ducts`. The four answers are identical to the element, which points at the second |
