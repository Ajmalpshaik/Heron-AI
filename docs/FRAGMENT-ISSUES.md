...
| **117** | **A WRITE THAT RENAMED NOTHING SAID "THE MODEL WAS CHANGED", AND [ROW 111's](#) FIX IS WHY ANYONE CAN SEE IT.** Measured 2026-09-17 on `test projject`, Revit 2024, verifying row 111 live. `RENAME_ELEMENTS` was run through `revit_change` with `find=ZZZNOTHINGMATCHESTHIS` against a 10-element selection. It reported `notMatched 10`, `planned 0`, **`renamed 0`** - correct, nothing matched - and then: **"the model was CHANGED and this is one undo step - Ctrl+Z in Revit puts it back"**. Nothing was changed. `WithVerdict` writes that sentence from **`applied`**, which is the caller's REQUEST to keep the transaction, not a statement that any work was done. **IT IS THE SAME FAMILY AS ROW 111 AND THE SAME FAMILY AS THE ROLLBACK FAILURES**: a write describing its INTENT where a reader will take it as an OUTCOME. `WithVerdict`'s own comment records the last time that cost something - *"on 2026-09-09 a rollback did not hold and this sentence claimed it had"* - and the `rolledBack` branch was fixed then to report what Revit said rather than what was asked. **THE `applied` BRANCH WAS LEFT ON THE REQUEST.** Harmless where work happened, misleading where none did, and a modeller reading only the last line would go looking for an undo step that has nothing under it. **IT IS ONLY VISIBLE BECAUSE ROW 111 SHIPPED THE SAME DAY** - before that, this whole reply was the single sentence *"RENAME_ELEMENTS ran in test projject."* and the contradiction had nowhere to show. | **OPEN.** Found 2026-09-17, by the row 111 fix working. The repair is to say what was KEPT rather than what was asked - the counts are already in the same reply, so the verdict can read them. Low severity and worth doing: every argument in `WithVerdict`'s own comment applies to it **THE REPAIR AS WRITTEN IS NOT AVAILABLE, AND THE REASON IS WORTH MORE THAN THE ROW.** Read 2026-09-19 without a Revit. *"the counts are already in the same reply, so the verdict can read them"* is true about the NUMBERS and false about the JUDGEMENT. `WithVerdict(answer, applied, name, rolledBack)` at `revit/Heron.Revit.Addin/RevitFragment.cs:1067` receives the answer as a **serialized JSON string** and appends to it; to say whether work happened it would have to parse that string and then decide **which key counts as work** - and that decision is `role: accounting` plus the bookkeeping patterns, which live in `fragment.yaml` and in `brain/heron_validate.looks_empty`. **THE ADD-IN HAS NEVER SEEN `fragment.yaml`.** `needs_for()` in `mcp/client/heron_bridge_client.py:854` sends the source and **the `needs` block only**; `provides`, with every `role:` on it, never crosses the wire. So the executor could read `renamed 0` and could not know that `renamed` is the result while `notMatched` is accounting - which is exactly the distinction [D-52](DECISIONS.md) exists for and exactly the one `batch-prove` imports `looks_empty` rather than re-implementing. **THREE PLACES THE REPAIR COULD LIVE, AND THEY ARE NOT THE SAME SIZE.** (1) **Send `provides` with the source**, the way `needs` already goes, and let the executor apply `role:` - the complete fix, and a change to the wire, the client and the executor at once. (2) **Write the sentence on the Python side**, where the judgement already lives - but `verdict` is a field the add-in puts INTO the reply, so this means the host trusting a second opinion over the one in the JSON, which is the drift this repository argues against everywhere else. (3) **Stop claiming what is not known**: one string, no new data, no wire change. `applied` is a fact about the TRANSACTION and the sentence should be about the transaction - *the change was committed and this is one undo step* - rather than *the model was CHANGED*, which is a claim about work. **(3) is the smallest honest repair and it is still not free**: it is user-facing wording in the add-in, governed by `.claude/skills/revit-addin-conventions/SKILL.md`, and **nothing in this container can run it** - C# compiles in CI and behaves only in front of a Revit. **NOTHING WAS CHANGED HERE.** AGENTS.md is explicit that a defect found while doing something else is recorded rather than fixed, and an untested wording change on the write path is the shape [row 111](#) and the rollback failures already cost this repository once each |
