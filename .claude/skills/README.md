...
| [heron-session](heron-session/SKILL.md) | **Hooks, not a document.** Before a pull request is merged or marked ready, fetches `origin/main` and tells the AI which commits the branch does not have. **Advice only — it never blocks.** Wired from [`../settings.json`](../settings.json) like the guard |
