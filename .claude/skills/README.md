...
| [heron-session](heron-session/SKILL.md) | **Hooks, not a document.** At every session start, one line: where the branch stands against `origin/main`, and how many fragments are PROVEN and DRAFT. Before a pull request is merged or marked ready, fetches `origin/main` and tells the AI which commits the branch does not have. **Advice only — neither ever blocks.** Wired from [`../settings.json`](../settings.json) like the guard. Also the diary every Heron hook writes one line per decision to, outside the repository, and `tools/hook-report.py` to read it |
