...
| [heron-ship](heron-ship/SKILL.md) | **What to run before pushing, in what order, and which failures are the machine rather than the change** — stating a change's intent and its before/after evidence, the four gates you run and the **nine CI decides on**, the reports whose findings are questions, the checker whose exit code follows the unfinished list, and the six that need a compiler or a store this container has not got |
