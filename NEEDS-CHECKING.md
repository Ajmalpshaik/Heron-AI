...
| **A9** | On any machine with the .NET SDK, run `python tools/check-fragments-compile.py` and read the result for **every** release | Every fragment compiles, as the tool itself reports - do not carry a number here, read its output. **This row exists because EIGHTY fragments were written on 2026-09-02/03, 2026-09-04 and 2026-09-05 in containers with no .NET SDK, and none could be installed:** the SDK download host is refused by those networks' policy, which answers `403` to the CONNECT itself - re-checked on 2026-09-04 against `builds.dotnet.microsoft.com`, and the proxy names the denial rather than timing out, so this is policy and not a slow link. They are `CREATE_PIPE`, `SET_MEP_SLOPE`, `CONNECT_OPEN_ENDS`, `FIND_DUPLICATE_VALUES`, `FIND_OVERLAPPING_TAGS`, `COLOR_BY_PARAMETER`, `ADD_PROJECT_PARAMETER`, `DUPLICATE_TYPE`, `CREATE_CEILING`, `CREATE_ELECTRICAL_RUN`, `JOIN_GEOMETRY`, `CREATE_WORKSET`, `CREATE_SELECTION_FILTER`, `SET_VIEW_UNDERLAY`, `RELOAD_LINKS`, `REPLACE_MATERIAL`, `AUTO_SIZE_MEP`, `CONNECT_AIR_TERMINALS`, `HIGHLIGHT_VS_REST`, `APPLY_VIEW_FILTER`, `CREATE_FLOOR`, `RENAME_FAMILY`, `CREATE_LEGEND_VIEW`, `CHECK_ROOM_MEP_COMPLETENESS`, `CREATE_WALL`, `CREATE_MATERIAL`, `REPORT_VIEW_FILTERS`, `SET_DATUM_BUBBLES`, `CREATE_FILLED_REGION`, `CREATE_KEY_SCHEDULE`, `FIND_OVERLAPPING_LINES`, `COPY_FROM_LINK`, `COPY_VIEW_FILTERS`, `REMAP_LINE_STYLES`, `SET_VIEW_WORKSET_VISIBILITY`, `CREATE_SHEET_LIST`, `REPORT_GLOBAL_PARAMETERS`, `CREATE_LINE`, `ASSIGN_SCOPE_BOX_TO_VIEW`, `SET_VIEW_CROP_TO_SHAPE`, `READ_GRAPHIC_OVERRIDES`, `CHECK_SLEEVE_SIZE`, `CHECK_EQUIPMENT_CLEARANCE`, `CHECK_VALVE_ACCESSIBILITY`, `CHECK_VERTICAL_CLEARANCE`, `AUDIT_MEP_OPENINGS`, `CREATE_ROOM_ELEVATIONS`, `CREATE_HVAC_ZONE`, `CHECK_FAMILY_STANDARDS`, `CHECK_CEILING_COORDINATION`, `CHECK_EQUIPMENT_CONNECTORS`, `CHECK_FIXTURE_CONNECTIVITY`, `REPORT_CATEGORY_OVERRIDES`, `REPORT_DESIGN_OPTIONS`, `RENAME_WORKSET`, `CREATE_GRIDS`, `CREATE_VIEW_FILTERS_BY_VALUE`, `REMOVE_VIEW_FILTER`, `REPORT_CATEGORY_VISIBILITY`, `SET_CROP_BOX_SETTINGS`, `CHECK_SURFACE_FIT`, `CREATE_MEP_SYSTEM_TYPE`, `REMOVE_PARAMETER_VALUE` and `ASSIGN_LOCATION_DATA`; and from 2026-09-04, the schedule batch - `REPORT_SCHEDULE_DEFINITION`, `READ_SCHEDULE_CONTENTS`, `REMOVE_SCHEDULE_FIELDS`, `SET_SCHEDULE_SORT_GROUP`, `SET_SCHEDULE_FILTERS`, `SET_SCHEDULE_APPEARANCE`, `EXPORT_SCHEDULE_TO_CSV` and `PLACE_SCHEDULE_ON_SHEET`; and from 2026-09-05, the reporting batch - `REPORT_COMPOUND_STRUCTURE`, `REPORT_CURTAIN_ELEMENTS`, `REPORT_DOOR_ROOM_LINKS`, `REPORT_BOUNDING_BOX`, `REPORT_AREAS`, `REPORT_PARAMETER_INVENTORY`, `REPORT_MEP_PRESSURE_DROP` and `REPORT_ROUTING_PREFERENCES`. **What was done instead is worth knowing, and worth not mistaking for a compile:** nuget.org IS reachable from there, so the Revit reference assemblies for 2020, 2024 and 2027 were downloaded and every API member used was looked up in their own metadata - type present, member present, argument COUNT. That catches the failure this repository has actually had (a member a release does not ship, like `Document.CreationGUID`) and it catches nothing else: **argument TYPES and plain C# syntax are unchecked**, and no metadata read substitutes for a compiler. **It has now caught nine real ones before they were written**: `ExternalDefinitionCreationOptions.Type` is gone by 2024; `Ceiling.Create` does not exist on 2020 at all; and `VariesAcrossGroups` is on `InternalDefinition` rather than `Definition`, so the obvious line compiles on no release. The fourth is a member that exists on ONE END ONLY and is the reverse shape of the others: `GlobalParameter.IsValidDataType` is present on 2020 and gone by 2024, so a reasonable-looking guard written against the oldest release would have turned every newer build red - a check that reads only the release in front of it finds nothing wrong. The fifth is a whole CAPABILITY a release takes away rather than a member: Revit **2027 removes HVAC zone creation from the API entirely** - the creation factory has no zone method left and `Zone.AddSpaces` is gone, while `Zone` itself remains, so the type existing proves nothing. `CREATE_HVAC_ZONE` reaches both vanished calls BY NAME for that reason: naming either directly would break the 2027 build, and a try/catch does not help because it never gets to run. The sixth is the same trap one level down, and it was caught in code this session had already written: `RENAME_WORKSET` reached `DeleteWorkset` **by name** because that call is absent from 2020 - and then named `DeleteWorksetSettings` in the reflection lookup's argument list, **a type that does not exist on 2020 either**. The reflection was protecting the build from one missing member while breaking it on another, on the very line meant to protect it. **Reaching a call by name is not enough: every TYPE in the lookup has to exist on the oldest release too.** The seventh is a third shape again, and the subtlest: an OVERLOAD whose ARITY changed. `ParameterFilterRuleFactory.CreateEqualsRule` for a text value is `(ElementId, string, bool)` on Revit 2020 and `(ElementId, string)` by 2027 - the case-sensitivity argument was dropped. **Each spelling compiles on exactly one end and breaks the other**, and nothing about the member's NAME hints at it; only reading both signatures does. `CREATE_VIEW_FILTERS_BY_VALUE` picks the overload at run time by its argument count. **A member being present at both ends does not mean the call is.** **The eighth, found 2026-09-04, is the one an existence check CANNOT reach at all: a property whose TYPE is ambiguous by name.** `ScheduleField.HorizontalAlignment` looks like it takes `HorizontalAlignmentStyle`, and both that enum and `ScheduleHorizontalAlignment` exist on every release from 2020 to 2027 - so every name check passes on both, and exactly one of them compiles. It was settled by decoding the property signature's `TypeDefOrRef` token out of the metadata blob and resolving it back to a row in the TypeDef table: `ScheduleHorizontalAlignment` on 2020 and on 2027. **Where two plausible types both exist, presence proves nothing and the signature has to be read.** **And validate the checker in both directions**: this session's own probe reported `Autodesk.Revit.DB.ScheduleFilterType` missing on all three releases when it is present on all of them - it was splitting a bare type name into a type plus a member. A checker that finds nothing is evidence about the checker; one that finds something FALSE is the same lesson with the sign flipped, and it is the more convincing of the two because it looks like a result. **The ninth, found 2026-09-05, is the version break that matched itself end to end:** `Definition.ParameterGroup` exists on 2020 and is GONE by 2027, and its replacement `Definition.GetGroupTypeId` has never existed on 2020 - a matched pair where neither spelling works at both ends, so `REPORT_PARAMETER_INVENTORY` reports no parameter group at all and says why. **A member with a replacement is not automatically portable; check the replacement against the OLD release too.** The same pass also corrected two names before they were written: `MEPSection` is in `Autodesk.Revit.DB.Mechanical` for PIPE systems as well as duct ones, and `RoutingPreferenceRule` has `GetCriterion(int)` rather than a `GetCriteria()` collection - neither guess exists on any release. **Walk the base types when checking** - `Space` declares neither `Number` nor `Area`, and `HostObjAttributes` does not declare `FamilyName`; both inherit them, and a check that stops at the declared type reports a false absence. **And match the FULL type name**: several short names resolve first to an internal marshalling struct rather than to the real class - `OverrideGraphicSettings` reads as having no constructors at all that way, and it has two. So this is the FIRST thing to run on a machine that has one, before any Revit work - a compile error found here costs a minute and the same error found at the PC costs a round trip through Revit |
...
| **B2** | Look at the panel | **Two** controls: **Heron** with an arrow under it, and Emergency Stop |
| **B2a** | Click the arrow under Heron | The list opens with **Bridge Status** in it |
| **B2b** | Pick Bridge Status, then look at the top of the split button | Still **Heron**, still showing the connected / disconnected picture. If the top has become Bridge Status, `IsSynchronizedWithCurrentItem` is not taking effect and the state indicator is gone |
