...
| **A5** | `python tools/check-compile.py 2025 2026 2027` — **on Windows** | Builds. These three target `net8.0-windows` / `net10.0-windows` with WPF, so they need the Windows Desktop SDK and are reported **SKIPPED** off Windows. **A skip is not a pass**, and this row exists so it never reads as one. 2027 also needs the .NET 10 SDK |
