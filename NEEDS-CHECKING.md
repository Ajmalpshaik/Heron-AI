...
| **A9** | On any machine with the .NET SDK, run `python tools/check-fragments-compile.py` and read the result for **every** release | Every fragment compiles, as the tool itself reports - do not carry a number here, read its output. **This row exists because eight fragments were written on 2026-09-02 in a container with no .NET SDK, and none could be installed:** the SDK download host is refused by that network's policy, which answers `403` to the CONNECT itself. They are `CREATE_PIPE`, `SET_MEP_SLOPE`, `CONNECT_OPEN_ENDS`, `FIND_DUPLICATE_VALUES`, `FIND_OVERLAPPING_TAGS`, `COLOR_BY_PARAMETER`, `ADD_PROJECT_PARAMETER` and `DUPLICATE_TYPE`. **What was done instead is worth knowing, and worth not mistaking for a compile:** nuget.org IS reachable from there, so the Revit reference assemblies for 2020, 2024 and 2027 were downloaded and every API member used was looked up in their own metadata - type present, member present, argument COUNT. That catches the failure this repository has actually had (a member a release does not ship, like `Document.CreationGUID`) and it catches nothing else: **argument TYPES and plain C# syntax are unchecked**, and no metadata read substitutes for a compiler. So this is the FIRST thing to run on a machine that has one, before any Revit work - a compile error found here costs a minute and the same error found at the PC costs a round trip through Revit |
