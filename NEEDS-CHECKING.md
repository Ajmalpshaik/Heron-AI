...
| **A4** | `dotnet build tests/Heron.Bridge.TestHost -p:RevitVersion=2024` then `python tests/test_bridge_roundtrip.py` — **on Windows** | **Mostly done 2026-08-28, and the remainder genuinely needs Windows.** All 32 checks pass on Linux: framing, the JSON parser, the token, newest-connection-wins, the toggle cycle and **the whole lease**. What a Linux run cannot touch is the Windows named pipe itself — its naming, its security descriptor, and the `CreateNewInstance` flag from [HANDOVER](HANDOVER.md) §4 note 2. Run it once on Windows and this row goes. **Build it for the runtime that is actually installed**: the command above pins `-p:HeronTfm=net8.0`, and on a box carrying only the .NET 10 runtime the host builds and then refuses to start — *"You must install or update .NET"*, which reads like a broken test rather than a missing runtime. `-p:HeronTfm=net10.0` runs it. Re-run on Linux 2026-08-29: **all 32 checks pass**, lease included |
...
| **A7** | On a machine that can reach a model host, `pip install --user model2vec` then `python brain/heron_embed.py "stop the air going the wrong way"` | The first line reads **`Backend: model`**, not `lexical`, and the duct fragment comes back for a sentence that contains none of its words. **Step 10 shipped with two backends and only one of them has ever run.** The built-in `lexical` one works offline with nothing installed and is measured, in numbers, as *not meaning* — synonyms score zero. The trained one is the half that understands a grille and a diffuser are cousins, and it could not be tested where it was written: `huggingface.co` is refused by that container's network policy (`connect_rejected`, measured 2026-08-28). **This needs no Revit and no Windows** — but *"a machine with a network"* is now known to be the wrong requirement, and the row said that until 2026-08-30. **Attempted again that day, in a different container, and it got further before hitting the same wall.** `pip install --user model2vec` **succeeded** — PyPI is reachable, the package resolves, and sixteen dependencies install clean — so the install step is no longer a variable and nobody need spend a run on it again. The block is specifically the **weights fetch**: `huggingface.co:443` answered **403 to CONNECT**, which that proxy records in its own words as *"policy denial"*, and its documentation says to report such a host rather than route around it. So the requirement is **a machine whose egress policy permits the model host**, which is a different and much rarer thing than a working network — two independent containers have now failed it for two different reasons. The owner's own PC is the obvious candidate. Note also that the `sentence_transformers` fallback in [`brain/heron_embed.py`](brain/heron_embed.py) fetches from **the same host**, so it is not a second route; and because `_load_model()` swallows every exception by design, a blocked download and an uninstalled package are indistinguishable to a reader — `backend()` correctly says *which* backend answered, never *why* the other one did not |
| **A9** | On any machine, `python tools/check-api-surface.py`, then read the 2020-2024 block | It reports **`MISSING TYPE Autodesk.Revit.UI.ItemData` on 2020 through 2024 and OK on 2025+`**, and nobody has yet established whether that is a real cross-version break or an artefact of the checker. **No `.cs` file in this repo names `ItemData`** - it is reached through a method signature (`RibbonPanel.AddItem` takes an `ItemData`), which is exactly the kind of reference this checker exists to find and a grep cannot. Two things point at an artefact rather than a defect: the string is present inside the cached 2020 `RevitAPIUI.dll`, and **`check-compile.py` builds the add-in clean on 2020** - if the type were genuinely absent there, that build could not succeed. But 'points at' is not 'establishes', and the two tools read different assemblies on purpose: the compile reads NuGet reference packages, this reads the shipped ones, and a reference assembly can carry a type the shipped one does not expose. **The checker exits 0**, so this is a report nobody is forced to look at, which is the condition under which a real finding gets ignored for months. Resolve it either way and say which: if the type is genuinely there, the checker is over-reporting and should stop; if it is genuinely missing, the add-in has a load-time break on five of its eight supported releases and no test would catch it. Found 2026-08-31 while adding fragments; **needs no Revit and no Windows** |
