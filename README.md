...
| **a BIM modeller** | [01 — Vision & Principles](docs/01-vision-and-principles.md), then the Status section above. Reading a model is proven far more widely than changing one — **144 `READ` fragments are `PROVEN` against 169 `MODIFY`**, and `write.enabled` still defaults to `false` |
