...
| Open questions | **42 answered · 6 open** — `Q-43` to `Q-47`, all opened 2026-09-09 **by tools asking questions nobody had asked**: the utterance cache is never written, the brain records none of its own answers, the model router may belong entirely to the host, 143 fragments name nothing they turned down, there are two capability-gap paths of which only one can fire, and **62 reading fragments collect only the host document in a world where models are federated**. **Nothing gates any phase** — [OPEN-QUESTIONS.md](docs/OPEN-QUESTIONS.md) |
