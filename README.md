...
| Open questions | **42 answered · 9 open** — `Q-43` to `Q-48`, all opened 2026-09-09 **by tools asking questions nobody had asked**: the utterance cache is never written, the brain records none of its own answers, the model router may belong entirely to the host, 59 fragments name nothing they turned down, there are two capability-gap paths of which only one can fire, and **62 reading fragments collect only the host document in a world where models are federated**. `Q-49` to `Q-51` came the same day from **reading someone else's repository at file level** ([33 §5](docs/33-external-repository-research.md)): Heron has no hooks, a preview tells the modeller what would change when it could show them, and **nothing yet guards the path from retrieval into context on the day Heron indexes text it did not write**. **Nothing gates any phase** — [OPEN-QUESTIONS.md](docs/OPEN-QUESTIONS.md) |
